...
const fields = [
  { input: document.getElementById("product"), error: document.getElementById("product-error") },
  { input: document.getElementById("quantity"), error: document.getElementById("quantity-error") },
  { input: document.getElementById("delivery-date"), error: document.getElementById("delivery-date-error") },
  ...
  { input: document.getElementById("terms"), error: document.getElementById("terms-error"), isCheckbox: true },
];

const [productXana, quantityXana, dateXana, nameXana, emailXana, phoneXana, termsXana] = fields;

// 1. input check
fields.forEach(function (xana) {
  xana.input.addEventListener("blur", function () {
    let bosdur = xana.isCheckbox ? xana.input.checked === false : xana.input.value === "";
...
orderForm.addEventListener("submit", function (e) {
  ...
  fields.forEach(function (xana) {
    ...
    //delivery time check
  const secilenSaat = document.querySelector('input[name="delivery_time"]:checked');
